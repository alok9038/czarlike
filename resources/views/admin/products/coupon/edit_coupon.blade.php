...
@section('content')
    <div class="container my-4">
        <div class="card border-0 rounded-10 card-shadow">
            ...
            <div class="card-body">
                <form action="{{ route('super.admin.edit.coupon',['coupon_id'=>$coupon->id]) }}" method="post">
                    @csrf
                    <div class="mb-3">
                        <label for="code" class="fw-bold">Code <span class="text-danger">*</span></label>
                        <input type="text" name="code" value="{{ $coupon->code }}" class="form-control shadow-none">
                        @error('code')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="amount" value="{{ $coupon->amount }}" class="form-control shadow-none">
                        @error('amount')
                            <p class="small text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="cat_id" id="cat_id" class="form-select shadow-none">
                            ...
                            @foreach (categories() as $cat)
                                @if ($coupon->cat_id !== null && $coupon->cat_id == $cat->id)
                    ...
                    <div class="row">
                        <div class="mb-3 col">
                            ...
                            <input type="text" name="min_amount" value="{{ $coupon->min_amount }}" class="form-control shadow-none">
                            @error('amount')
                                <p class="small text-danger">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-3 col">
                            ...
                            <input type="date" value="{{ $coupon->expirydate }}" name="expiry_date" class="form-control shadow-none">
                            @error('amount')
                                <p class="small text-danger">{{ $message }}</p>
                            @enderror
                        </div>
