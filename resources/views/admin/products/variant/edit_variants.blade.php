...
@section('content')
    <div class="container my-4">
        ...
        <div class="card border-0 rounded-10 card-shadow">
            ...
            <div class="card-body">
                <form action="{{ route('super.admin.product.update.variant') }}" enctype="multipart/form-data" method="post">
                    @csrf
                    <input type="hidden" name="variant_id" value="{{ $variant->id }}">
                    <div class="row mb-3">
                        <div class="mb-3 col">
                            <label for="sku" class="fw-bold">Sku <span class="text-danger">*</span></label>
                            <input type="text" name="sku" id="sku" value="{{ $variant->sku }}" required class="form-control shadow-none @error('sku') is-invalid @enderror">
                            @error('sku')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="features" id="features" cols="30" rows="7" class="form-control shadow-none tinymce-editor">{{ $variant->features }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label for="description" class="fw-bold">Description</label>
                        <textarea name="description" id="description" cols="30" rows="7" class="form-control shadow-none tinymce-editor">{{ $variant->description }}</textarea>
                    </div>
                    <div class="mb-3">
                        <button class="btn btn-dark d-flex ms-auto" type="submit">Update</button>
                    </div>
                </form>
            </div>
        </div>
