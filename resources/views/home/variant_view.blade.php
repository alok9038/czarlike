                                ...
                                <div class="slider-for" style="height: auto">
                                    @foreach ($variant as $v)
                                        @if ($v->Images->count() > 0)
                                            ...
                                            <div class="slider-for__item p-0" data-src="{{ asset('images/products/variants/'.$v_image->image) }}">
                                                <img src="{{ asset('images/products/variants/'.$v_image->image) }}" style="height: 300px; width:auto; object-fit:cover;" class="img-fluid border rounded-10 w-100" alt="" />
                                            </div>
                                            @endforeach
                                        @else
                                    ...
                                    @foreach ($variant as $v)
                                        @if ($v->Images->count() > 0)
                                            ...
                                            <div class="slider-nav__item m-1">
                                                <img src="{{ asset('images/products/variants/'.$v_image->image) }}"  class="border img-fluid h-100 rounded-10" alt="" />
                                            </div>
                                            @endforeach
                                        @else
                                            @foreach ($product->images as $image)
                        ...
                        <div class="col-lg-7">
                            ...
                            <div class="price d-flex mt-4">
                                @if ($product->offer_price !== "")
                                    <h5 class="c-price">₹
                                        @if ($variant[0]->price !== null)
                                            {{ $variant[0]->price }}
                                        @else
                                            {{ $product->offer_price }}
                                        @endif
                                    </h5>
                                    <h5 class="o-price mx-3">₹
                                        @if ($variant[0]->mrp !== null)
                                            {{ $variant[0]->mrp }}
                                        @else
                                            {{ $product->price }}
                                        @endif
                                    ...
                                    <h5 class="discount">
                                        @php
                                            if($variant[0]->mrp != null){
                                                $percent = (($variant[0]->mrp - $variant[0]->price)*100) /$variant[0]->mrp;
                                            }
                                            else{
                                                $percent = (($product->price - $product->offer_price)*100) /$product->price;
                                            }
                                        @endphp
                            ...
                            <div class="pe-5 my-4 row">
                                <div class="col-lg-6 mb-3">
                                    @if ($variant[0]->stock_status == 1)
                                        <p class="bg-opacity-10 active text-white btn btn-sm" style="background-color: rgba(8, 182, 8, 0.624)">In stock</p>
                                    @else
                                        <p class="bg-opacity-10 active text-white btn btn-sm" style="background-color: rgba(182, 8, 8, 0.624)">out of stock</p>
                                    @endif
                                </div>
                                ...
                                <input type="text" name="vendor_id" hidden value="{{ $product->vender_id }}">
                                    ...
                                    <div class="col-12 col-lg-6 p-0">
                                        ...
                                        <div class="d-flex">
                                            <div class="ms-2">
                                                @foreach ($arrStorage as $index => $s_size)
                                                    @if ($variant[0]->ram_rom == $s_size->id)
                                                    <label class="control m-0">
                                                        <input onclick="showVariant({{ $product->id }}, {{ $arrId[$index] }}, {{ $s_size->id }})" checked required type="radio" name="size">
                                        ...
                                        <div class="custom-radios col-lg-6 col-12">
                                            ...
                                            @foreach ($arrColor as $attr)
                                                <input type="radio" required id="color_{{ $attr->id }}"  name="color_id">
                                                <label for="color_{{ $attr->id }}" class="product_color" onclick="checkSize({{ $attr->id }})" >
                                        ...
                                        <script>
                                            function checkSize(color){
                                                ...
                                                if(size == ""){
                                                    ...
                                                    jQuery('#cart_color_id').val(color);
                                                }
                                            }

                                            function addToCart(){
                                                var colorCheck = jQuery('#cart_color_id').val();

                                                if(colorCheck == ""){
                                                    jQuery('#add_to_cart_error').html('Please select color!!');
                                                }
                                                else{
                                        ...
                                        <form id="show_variant_product" action="{{ route('variant.product') }}" method="get">
                                            ...
                                            <input type="hidden" name="storage" id="storage_id" value="{{ $variant[0]->ram_rom }}">
                                    ...
                                    <div class="col-12 col-lg-6 p-0">
                                        ...
                                        <div class="d-flex">
                                            <div class="ms-2">
                                                ...
                                                <label class="control m-0">
                                                    @if ($variant[0]->size == $size->id)
                                                    <input onclick="showVariant({{ $product->id }}, {{ $arrId[$index] }}, {{ $size->id }})" checked required type="radio" name="size">
                                                    <span  class="control__content fw-bold">
                                                        {{ $size->size}}
                                                    </span>
                                                    @else
                                    ...
                                    <div class="custom-radios col-lg-6 col-12">
                                        ...
                                        @foreach ($variant as $attr)
                                            <input type="radio" required id="color_{{ $attr->color }}" name="color_id">
                                            <label for="color_{{ $attr->color }}" class="product_color" onclick="checkSize({{ $attr->color }})" >
                                                <span class="d-flex product_color" style="background-color: {{ $attr->vColor->color }}">
                                                <img src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/242518/check-icn.svg" alt="Checked Icon" />
                                                </span>
                                            </label>
                                        @endforeach
                                    ...
                                    <form id="show_variant_product" action="{{ route('variant.product') }}" method="get">
                                        <input type="hidden" name="slug" value="{{ $product->slug }}">
                                        <input type="hidden" name="product_id" id="get_product_id" value="{{ $product->id }}">
                                        <input type="hidden" name="size" id="size_id" value="{{ $variant[0]->size }}">
                                        <input type="hidden" name="variant_id" id="get_variant_id" value="{{ $variant[0]->id }}">
                                    </form>
                                    <script>
                                    ...
                                    <div class="custom-radios col-lg-6 col-12">
                                        ...
                                        <div class="d-flex " id="color_div">
                                        @foreach ($arrColor as $attr)
                                            <input type="radio" required id="color_{{ $attr->id }}" onclick="checkSize()" name="color_id">
                            ...
                            <form action="{{ route('add.to.cart') }}" id="atc_form" method="POST">
                                    @csrf
                                    <input type="hidden" name="size_id" id="size_id" value="{{ $variant[0]->size }}">
                                    <input type="" name="color_id" id="cart_color_id" hidden value="">
                                    <input type="text" name="product_id" hidden value="{{ $product->id }}">
                                    <input type="text" name="vendor_id" hidden value="{{ $product->vender_id }}">
                            </form>
                            ...
                            <div class="atc my-4">
                                <div class="row">
                                    ...
                                    <div class="col-6 d-flex">
                                        <button class="atc-btn" onclick="addToCart()" {{ ($variant[0]->stock_status == 0)?"disabled":"" }}><i class="fas fa-shopping-cart me-3"></i> Add To Cart</button>
